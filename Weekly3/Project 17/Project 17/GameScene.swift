...
//  Project 17
//
//  Created by René Melo de Lucena on 12/03/19.
...
class GameScene: SKScene {
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for node in nodesAtPoint {
            if node.name == "enemy" {
                ...
                let index = activeWords.firstIndex(of: node as! SKSpriteNode)!
                activeWords.remove(at: index)
                ...
                let index = activeWords.firstIndex(of: node.parent as! SKSpriteNode)!
                activeWords.remove(at: index)
            ...
            for node in activeWords {
                if node.position.y < -140 {
                    ...
                    if node.name == "enemy" {
                        ...
                        if let index = activeWords.firstIndex(of: node) {
                            activeWords.remove(at: index)
                        }
    ...
    func endGame(triggeredByBomb: Bool) {
        ...
        let endGameLabel = SKLabelNode(text: "END GAME")
        endGameLabel.color = UIColor(named: "white")
        endGameLabel.fontName = "Chalkduster"
        endGameLabel.position = CGPoint(x: self.frame.width/2, y: self.frame.height/2)
        endGameLabel.fontSize = 128
